// StockTransferProductCard.tsx — mahsulot kartasi komponenti
...
interface StockTransferProductCardProps {
  readonly item:     StockLevel;
  readonly onSelect: (item: StockLevel) => void;
}

export const StockTransferProductCard = React.memo(function StockTransferProductCard({
  item,
  onSelect,
}: StockTransferProductCardProps) {
  ...
  return (
    <TouchableOpacity
      style={styles.card}
      onPress={() => !isOut && onSelect(item)}
      activeOpacity={isOut ? 1 : 0.75}
    >
      {/* Yuqori qator: mahsulot nomi + status badge */}
      ...
      {/* Pastki qator: qoldiq + o'tkaz tugmasi */}
      <View style={styles.bottomRow}>
        ...
        <TouchableOpacity
          style={[styles.selectBtn, isOut && styles.selectBtnDisabled]}
          onPress={() => onSelect(item)}
          activeOpacity={0.75}
          disabled={isOut}
        >
          <Ionicons name="swap-horizontal-outline" size={14} color={C.white} />
          <Text style={styles.selectBtnText}>O'tkaz</Text>
        </TouchableOpacity>
      </View>
    </TouchableOpacity>
  );
