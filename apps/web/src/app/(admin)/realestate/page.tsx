...
export default function RealEstatePage() {
  ...
  const STATUS_CONFIG: Record<PropertyStatus, { label: string; className: string }> = {
    RENTED: { label: t('realestate.statusRented'), className: 'bg-green-100 text-green-700' },
    VACANT: { label: t('realestate.statusVacant'), className: 'bg-yellow-100 text-yellow-700' },
    MAINTENANCE: { label: t('realestate.statusMaintenance'), className: 'bg-red-100 text-red-700' },
  };

  const TYPE_LABELS: Record<PropertyType, string> = {
    OFFICE: t('realestate.typeOffice'),
    WAREHOUSE: t('realestate.typeWarehouse'),
    RETAIL: t('realestate.typeShop'),
    APARTMENT: t('realestate.typeApartment'),
  };

  const PAYMENT_STATUS_CONFIG: Record<PaymentStatus, { label: string; className: string }> = {
    PAID: { label: t('realestate.paymentPaid'), className: 'bg-green-100 text-green-700' },
    PENDING: { label: t('realestate.paymentPending'), className: 'bg-yellow-100 text-yellow-700' },
    OVERDUE: { label: t('realestate.paymentOverdue'), className: 'bg-red-100 text-red-700' },
  };
  ...
  return (
    <div className="flex flex-col gap-6 p-6">
      ...
      <div>
        ...
        <p className="mt-0.5 text-sm text-gray-500">{t('realestate.subtitle')}</p>
      </div>
      ...
      {statsLoading ? (
        ...
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          <StatCard label={t('realestate.totalProperty')} value={stats.totalProperties} icon={Building2} color="bg-blue-600" />
          <StatCard label={t('realestate.rented')} value={stats.rented} icon={Users} color="bg-green-600" />
          <StatCard label={t('realestate.monthlyRent')} value={fmtPrice(stats.totalMonthlyRent)} icon={DollarSign} color="bg-violet-600" />
          <StatCard
            label={t('realestate.overdue')}
        ...
        <>
          ...
          {propsLoading ? (
            ...
            <ErrorState
              ...
              title={t('realestate.backendNotReady')}
              description={t('realestate.backendNotReadyDesc')}
            />
          ) : filteredProps.length === 0 ? (
            <EmptyState
              icon={Building2}
              title={t('realestate.noProperties')}
              description={search ? t('realestate.noSearchResults') : t('realestate.noPropertiesYet')}
            />
          ) : (
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-3 grid grid-cols-2 gap-2 text-sm">
                      <div>
                        <p className="text-xs text-gray-400">{t('realestate.rentPrice')}</p>
                        <p className="font-semibold text-gray-900">{fmtPrice(p.rentAmount)}</p>
                      </div>
                      {p.area && (
                        <div>
                          <p className="text-xs text-gray-400">{t('realestate.area')}</p>
        ...
        <>
          <div className="flex gap-1">
            {(['ALL', 'PENDING', 'OVERDUE', 'PAID'] as const).map((s) => (
              ...
              >
                {s === 'ALL' ? t('common.all') : PAYMENT_STATUS_CONFIG[s].label}
              </button>
            ))}
          </div>
            ...
            <table className="w-full text-sm">
              <thead className="border-b border-gray-200 bg-gray-50">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left font-medium text-gray-600">{t('realestate.tenant')}</th>
                  <th className="px-4 py-3 text-left font-medium text-gray-600">{t('realestate.month')}</th>
                  <th className="px-4 py-3 text-right font-medium text-gray-600">{t('realestate.amount')}</th>
                  <th className="px-4 py-3 text-left font-medium text-gray-600">{t('realestate.deadline')}</th>
                ...
                {!payments || payments.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="py-12 text-center">
                      ...
                      <p className="text-sm text-gray-500">{t('realestate.noPaymentsYet')}</p>
                    </td>
                  </tr>
                ) : (
                  payments.map((pay) => {
