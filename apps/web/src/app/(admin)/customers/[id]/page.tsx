...
function DebtAgeBadge({ status, ageDays }: { status: Debt['status']; ageDays: number }) {
  ...
  const configs: Record<Debt['status'], { label: string; className: string }> = {
    ...
    OVERDUE_30: { label: `${ageDays} ${t('analytics.days')}`, className: 'bg-yellow-100 text-yellow-700' },
    OVERDUE_60: { label: `${ageDays} ${t('analytics.days')}`, className: 'bg-orange-100 text-orange-700' },
    OVERDUE_90: { label: `${ageDays} ${t('analytics.days')}`, className: 'bg-red-100 text-red-700' },
    OVERDUE_90PLUS: { label: `${ageDays} ${t('analytics.days')}`, className: 'bg-red-200 text-red-800' },
  };
...
function PayDebtModal({
  ...
  const METHODS: { key: PayMethod; label: string }[] = [
    { key: 'CASH', label: t('nasiya.methodCash') },
    { key: 'CARD', label: t('nasiya.methodCard') },
    { key: 'TRANSFER', label: t('nasiya.methodTransfer') },
  ];
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative z-10 w-full max-w-sm rounded-2xl bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4">
          <div>
            <h2 className="text-base font-semibold text-gray-900">{t('nasiya.payTitle')}</h2>
        ...
        <div className="p-5">
          ...
          <div className="mb-5">
            <label className="mb-1.5 block text-sm font-medium text-gray-700">
              {t('nasiya.payNoteLabel')}
            </label>
            <input
              type="text"
              value={note}
              onChange={(e) => setNote(e.target.value)}
              placeholder={t('nasiya.payNotePlaceholder')}
          ...
          >
            ...
            {isPending ? t('common.saving') : t('nasiya.paySubmit', { amount: formatPrice(amount) })}
          </button>
        </div>
      </div>
    </div>
  ...
  if (!customer) {
    return (
      <div className="flex flex-col items-center justify-center gap-4 p-12">
        <p className="text-gray-500">{t('customers.notFoundSingle')}</p>
        <Link
          href="/customers"
      ...
      >
            ...
            <div>
              ...
              <div className="mt-1 flex items-center gap-4 text-sm text-gray-500">
                ...
                <span className="flex items-center gap-1">
                  ...
                  {t('customers.purchaseCount', { count: customer.totalPurchases })}
                </span>
                <span className="flex items-center gap-1">
                  <UserCircle className="h-3.5 w-3.5" />
                  {customer.lastVisitAt
                    ? t('customers.lastVisitDate', { date: new Date(customer.lastVisitAt).toLocaleDateString('uz-UZ') })
                    : t('customers.noVisitYet')}
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="mt-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <div className="rounded-xl bg-white/80 p-4">
            <p className="text-xs text-gray-500">{t('customers.currentDebt')}</p>
          ...
          <div className="rounded-xl bg-white/80 p-4">
            <p className="text-xs text-gray-500">{t('customers.debtLimitTitle')}</p>
          ...
          <div className="rounded-xl bg-white/80 p-4">
            <p className="text-xs text-gray-500">{t('customers.overdueDebt')}</p>
      ...
      <div>
          ...
          <h2 className="font-semibold text-gray-900">
            ...
            {activeDebts.length > 0 && (
              <span className="ml-2 text-sm font-normal text-gray-500">
                ({t('customers.debtsSummary', { count: activeDebts.length, total: formatPrice(totalRemaining) })})
              </span>
            )}
          </h2>
        </div>
        ...
        {loadingDebts ? (
          ...
          <div className="rounded-xl border border-gray-200 bg-white px-6 py-10 text-center">
            ...
            <p className="text-sm text-gray-400">{t('customers.noActiveDebt')}</p>
          </div>
        ) : (
          <div className="overflow-hidden rounded-xl border border-gray-200 bg-white">
            <table className="w-full text-sm">
              <thead className="border-b border-gray-200 bg-gray-50">
                <tr>
                  <th className="px-4 py-3 text-left font-medium text-gray-600">{t('customers.colOrder')}</th>
                  <th className="px-4 py-3 text-right font-medium text-gray-600">{t('customers.colOriginalAmount')}</th>
                  <th className="px-4 py-3 text-right font-medium text-gray-600">{t('customers.colRemaining')}</th>
                  <th className="px-4 py-3 text-left font-medium text-gray-600">{t('customers.colDueDate')}</th>
                  <th className="px-4 py-3 text-center font-medium text-gray-600">{t('customers.colAge')}</th>
                  <th className="px-4 py-3 text-center font-medium text-gray-600">{t('customers.colAction')}</th>
                </tr>
              </thead>
