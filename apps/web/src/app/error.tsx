...
export default function Error({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center gap-4 p-8 text-center">
      {/* Brand mark — keeps user oriented on error pages */}
      ...
      <p className="max-w-sm text-sm text-gray-500">{error.message || 'Kutilmagan xatolik yuz berdi. Iltimos, qayta urinib ko\'ring.'}</p>
      <button
        onClick={reset}
