...
## T-064 | 2026-03-12 | [FRONTEND] | Sync status UI — Persistent status bar
- **Yechim:** SyncStatusBar.tsx (256 qator) + sync.store.ts amalga oshirilgan. 4 holat: online-synced (yashil), online-syncing (ko'k), offline (qizil), slow (sariq). Ping /api/v1/health/ping har 10s. Pending queue dropdown. ShiftBar ga ulangan.
- **Fayllar:** components/SyncStatus/SyncStatusBar.tsx, store/sync.store.ts

---

## T-110 | 2026-03-12 | [FRONTEND] | Thermal printer — ESC/POS integration
- **Yechim:** MVP — window.print() + 80mm ReceiptTemplate.tsx (147 qator). Auto-print setting (localStorage). useReceiptPrint hook. ReceiptPreview.tsx modal. Cash drawer auto-open. Tauri ESC/POS → T-049 da.
- **Fayllar:** components/Receipt/ReceiptTemplate.tsx, components/Receipt/useReceiptPrint.ts, app/(pos)/pos/ReceiptPreview.tsx

---

## T-117 | 2026-03-12 | [FRONTEND] | Customer display — 2-ekran (ikkinchi monitor)
- **Yechim:** BroadcastChannel API orqali POS → customer-display oynasiga real-time cart ma'lumoti uzatiladi. 3 holat: idle (xush kelibsiz), cart (real-time items), sale-complete (5s "Rahmat" ekrani). ShiftBar da "Mijoz ekrani" tugmasi — yangi oynada ochadi. usePOSKeyboard ga F9 (bonus) qo'shildi.
- **Fayllar:** hooks/pos/useCustomerDisplayBroadcast.ts, app/(pos)/pos/customer-display/page.tsx, app/(pos)/pos/ShiftBar.tsx, app/(pos)/pos/page.tsx, hooks/pos/usePOSKeyboard.ts

---

## T-044 | 2026-03-12 | [FRONTEND] | Loyalty UI — Customer points + redeem
- **Yechim:** types/loyalty.ts + api/loyalty.api.ts + hooks/customers/useLoyalty.ts yaratildi. PaymentPanel.tsx ga "Bonuslar" (F9, violet) 5-usul qo'shildi: customer tanlash → points ko'rsatish → ball kiritish → BONUS payment. useCompleteSale.ts da bonus canComplete + payments. customers/[id]/page.tsx da loyalty card (tier, points, totalEarned/Redeemed) + transactions table. PaymentEntryDto ga 'BONUS' qo'shildi. TypeScript: 0 xato.
- **Fayllar:** types/loyalty.ts, api/loyalty.api.ts, hooks/customers/useLoyalty.ts, types/sales.ts, store/pos.store.ts, hooks/pos/useCompleteSale.ts, app/(pos)/pos/PaymentPanel.tsx, app/(admin)/customers/[id]/page.tsx
...
| T-090 | 2026-03-02 | Analytics dashboard UI — 6 tabs: sotuv trendi (LineChart 30 kun), top mahsulotlar (horizontal BarChart + DataTable marja badge), marja tahlili (BarChart kategoriya, dual Y axis), kassirlar (samaradorlik: bekor/chegirma fraud detection), soatlik faollik (BarChart rang kodlash: qizil/sariq/ko'k), harakatsiz tovar (Dead stock: kun, qty, carrying cost), ABC tahlil (3 bucket progress bars), Analitika link Sidebar | `apps/web/src/app/(admin)/analytics/page.tsx`, `apps/web/src/components/layout/Sidebar.tsx` |
| T-111 | 2026-03-02 | Cash drawer — cashDrawer.ts utility (getPrinterSettings, openCashDrawer: network mode POST /drawer, browser mode simulated), isCashDrawerEnabled(), "Kassa" button in ShiftBar (faqat isCashDrawerEnabled() bo'lganda ko'rinadi), auto-open in ReceiptPreview (CASH payment detect) | `apps/web/src/lib/cashDrawer.ts`, `apps/web/src/app/(pos)/pos/ShiftBar.tsx`, `apps/web/src/app/(pos)/pos/ReceiptPreview.tsx` |
| T-112 | 2026-03-12 | Label printer UI — LabelPrintModal kengaytirildi: 3 o'lcham tanlash (30×20/40×30/58×40mm), printer modeli dropdown (Zebra/TSC/XPrinter/RONGTA), muddat ko'rsatish toggle, preview real o'lchamda ko'rsatadi, har mahsulotga nusxa soni, window.open→print CSS, Product type ga expiryDate? optional field qo'shildi | `apps/web/src/app/(admin)/catalog/products/LabelPrintModal.tsx`, `apps/web/src/types/catalog.ts` |
| T-109 | 2026-03-12 | Billing UI — joriy obuna holati (STATUS badge, tugash sanasi, sinov banneri), foydalanish progress bars (filiallar/mahsulotlar/foydalanuvchilar, ≥80% orange), tarif kartalari (name/price/limits/features, joriy tarif belgilangan), upgrade/downgrade mutatsiya, useSubscription/useBillingPlans/useUsageStats/useUpgradePlan hooks, types/billing.ts, api/billing.api.ts | `apps/web/src/app/(admin)/settings/billing/page.tsx`, `apps/web/src/hooks/settings/useBilling.ts`, `apps/web/src/api/billing.api.ts`, `apps/web/src/types/billing.ts` |
| T-041 | 2026-03-12 | Supplier management UI — DataTable (name/company/phone/address), SupplierModal (create/edit: Zod validation), ConfirmDialog (delete), empty state, useSuppliers/useCreateSupplier/useUpdateSupplier/useDeleteSupplier hooks, types/supplier.ts, api/suppliers.api.ts, Sidebar link mavjud | `apps/web/src/app/(admin)/catalog/suppliers/page.tsx`, `apps/web/src/hooks/catalog/useSuppliers.ts`, `apps/web/src/api/suppliers.api.ts`, `apps/web/src/types/supplier.ts` |
| T-115 | 2026-03-02 | Branch reports UI — 3 filial demo (Chilonzor/Yunusobod/Sergeli), 7-kunlik BarChart (side-by-side, 3 rang), jami cards (daromad/buyurtmalar/foyda), filiallar DataTable (rang dot, ulush %, sortable: revenue/orders/profit/avgCheck), ko'chirma tarixi (transfer status: RECEIVED/SHIPPED/PENDING), Filiallar link reports Sidebar | `apps/web/src/app/(admin)/reports/branches/page.tsx`, `apps/web/src/components/layout/Sidebar.tsx` |
